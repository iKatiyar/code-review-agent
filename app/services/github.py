...
class GitHubService:
    ...
    def post_pr_review(
        ...
        """
        Post an AI-generated review summary as a GitHub PR comment.

        Uses ``create_review`` (event=COMMENT) so no approval/change-request
        is forced — the agent only surfaces findings.

        Args:
            repo_url: GitHub repository URL.
            pr_number: Pull request number.
            summary: Aggregated summary dict from the LangGraph synthesize node.
            file_issues: Per-file issue list keyed by file path.
        ...
        try:
            ...
            body = self._format_review_body(summary, file_issues)

            review = pull_request.create_review(body=body, event="COMMENT")

            logger.info(
                f"Posted review on PR #{pr_number} — review id {review.id}"
            )
            return {
            ...
            self._handle_github_exception(e, f"posting review on PR #{pr_number}")
        except (InvalidRepositoryException, GitHubAPIException):
            raise
        except Exception as e:
            raise GitHubAPIException(
                f"Failed to post review on PR #{pr_number}: {str(e)}",
                details={"repo_url": repo_url, "pr_number": pr_number},
            )

    def _format_review_body(
        self, summary: Dict[str, Any], file_issues: Dict[str, Any]
    ) -> str:
        """Format analysis results into a readable GitHub review comment."""
        severity = summary.get("severity_breakdown", {})
        total = summary.get("total_issues", 0)
        files_count = summary.get("total_files_analyzed", 0)
        ...
        lines = [
            ...
            "",
            f"Analyzed **{files_count} file(s)** and found **{total} issue(s)**.",
            "",
            "### Severity Breakdown",
            "",
        ]
        ...
        for level in ("critical", "high", "medium", "low"):
            ...
            if count:
                lines.append(f"- {emoji_map[level]} **{level.capitalize()}**: {count}")
        ...
        if file_issues:
            ...
            for file_path, data in file_issues.items():
                ...
                if not issues:
                    continue
                lines.append(f"<details><summary><code>{file_path}</code> — {len(issues)} issue(s)</summary>")
                lines.append("")
                for issue in issues:
                    sev = issue.get("severity", "low")
                    icon = emoji_map.get(sev, "•")
                    line_num = issue.get("line", "?")
                    desc = issue.get("description", "")
                    suggestion = issue.get("suggestion", "")
                    lines.append(f"**Line {line_num}** {icon} `{issue.get('type', '')}` — {desc}")
                    if suggestion:
        ...
        lines += [
            "---",
            "*Generated by [Code Review Agent](https://github.com/iKatiyar/code-review-agent)*",
        ]
